...
void DFS(const ADJ_MATRIX &graph, vector<ii> &spanningTree, vector<bool> &visit, int u)
{
    visit[u] = true;
    for (int v = 0; v < graph[u].size(); v++)
    {
        if (graph[u][v] > 0 && !visit[v])
        {
            ...
            DFS(graph, spanningTree, visit, v);
        }
    }
}
...
vector<ii> findSpanningTree(const ADJ_MATRIX &graph, int startVertex)
{
    ...
    DFS(graph, spanningTree, visit, startVertex);
    return spanningTree;
}
int main()
{
    ...
    cout << "ST found with DFS's algorithm:\n";
    printEdgesList(el);

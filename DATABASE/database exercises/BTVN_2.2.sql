...
where tl.Ten = N'nhạc cách mạng' and tg.HoTen = N'Đỗ Nhuận'
...
select tl.IDTheLoai, tl.Ten
from THELOAI tl
join BAIHAT bh on tl.IDTheLoai = bh.IDTheLoai

--Cho danh sách tác giả (mã, họ tên, số bài hát) của mỗi tác giả.
select tg.IDTacGia, tg.HoTen, COUNT(bh.IDBaiHat) as SoLuongBaiHat
from TACGIA tg 
join BAIHAT bh on tg.IDTacGia = bh.IDTacGia
